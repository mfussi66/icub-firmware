                     ...
                     ZCE_process_sensors_T *localZCE)
{
  ...
  *rty_SensorDataDecoded = *rtu_SensorData;
  ...
  if (true || rtu_Flags->enable_currents_bias_removal) {
    ...
    for (ForEach_itr = 0; ForEach_itr < 3; ForEach_itr++) {
      ...
      localB->CoreSubsys[ForEach_itr].gt = rtu_SensorData->
        motorsensors.Iabc[ForEach_itr]+off - localDW->CoreSubsys[ForEach_itr].
  ...
  for (ForEach_itr = 0; ForEach_itr < 3; ForEach_itr++) {
    rtb_Subtract[ForEach_itr] = rtu_SensorData->motorsensors.Iabc[ForEach_itr]+off -
  ...
  for (ForEach_itr = 0; ForEach_itr < 3; ForEach_itr++) {
    rty_SensorDataDecoded->motorsensors.Iabc[ForEach_itr] =
      rtb_Subtract[ForEach_itr]-rtb_bt;
  }

		 if((++cnt1 % 9001) == 0)
	{
		for (uint8_t i = 0; i < 3; i++) {
				embot::core::print("----" + std::to_string(i) + "----\n" + "Iabc " + std::to_string(rtu_SensorData->motorsensors.Iabc[i]+off));
